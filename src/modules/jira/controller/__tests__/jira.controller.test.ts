...
describe('JiraController', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    controller = JiraController.getInstance()
  ...
  it('registers atlassianEmail successfully', async () => {
    ...
    await controller.registerAtlassianEmail({
      payload: { text: '.jira me user@company.com', user: 'U123', channel_type: 'im', channel: 'C123' },
      say,
    })

    expect(updateUserByIdMock).toHaveBeenCalledWith(1, { atlassianEmail: 'user@company.com' })
    expect(say).toHaveBeenCalledWith(
      '✅ Tu email de Jira fue registrado correctamente: *user@company.com*'
    )
  })

  it('replies with error when no email is provided', async () => {
    await controller.registerAtlassianEmail({
      payload: { text: '.jira me', user: 'U123', channel_type: 'im', channel: 'C123' },
      say,
    })
  ...
  it('replies with error when email format is invalid', async () => {
    await controller.registerAtlassianEmail({
      payload: { text: '.jira me notanemail', user: 'U123', channel_type: 'im', channel: 'C123' },
      say,
    })
  ...
  it('replies with error when service returns an error', async () => {
    ...
    expect(say).toHaveBeenCalledWith('Ups! No se pudo registrar tu email de Jira 😢')
  })
